...
echo "=== Updating AssistantCore.java with Verified Model URL ==="
...
public class AssistantCore {
    ...
    public void downloadModel(DownloadCallback callback) {
        new Thread(() -> {
            try {
                // Verified robust download endpoint for MediaPipe compatible LLMs
                URL url = new URL("https://huggingface.co/litert-community/TinyLlama-1.1B-Chat-v1.0/resolve/main/tinyllama-1.1b-chat-v1.0-int4.task");
...
echo "Building and pushing update..."
gradle assembleDebug
adb install -r app/build/outputs/apk/debug/app-debug.apk
echo "=== Done! Download link updated with explicit redirect handling. ==="
